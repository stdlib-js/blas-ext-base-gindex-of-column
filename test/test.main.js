...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof gindexOfColumn, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 7', function test( t ) {
	t.strictEqual( gindexOfColumn.length, 7, 'returns expected value' );
	t.end();
});
...
tape( 'the function throws an error if provided a first argument which is not a valid order', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			gindexOfColumn( value, data.M, data.N, data.A, data.LDA, data.x, data.strideX );
		};
	}
});
...
tape( 'the function throws an error if provided a fifth argument which is not a valid `LDA` value (row-major)', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			gindexOfColumn( data.order, data.M, data.N, data.A, value, data.x, data.strideX );
		};
	}
});
...
tape( 'the function returns an invalid index when M is less than or equal to zero', function test( t ) {
	...
	data = ROW_MAJOR_DATA;
	out = gindexOfColumn( data.order, 0, data.N, data.A, data.LDA, data.x, data.strideX );
...
tape( 'the function returns an invalid index when N is less than or equal to zero', function test( t ) {
	...
	data = ROW_MAJOR_DATA;
	out = gindexOfColumn( data.order, data.M, 0, data.A, data.LDA, data.x, data.strideX );
...
tape( 'the function returns the index of the first column matching a search vector (row-major)', function test( t ) {
	...
	data = ROW_MAJOR_DATA;
	out = gindexOfColumn( data.order, data.M, data.N, data.A, data.LDA, data.x, data.strideX );
